<!DOCTYPE html>
<html>
<head>
	<link rel="stylesheet" type="text/css" href="../themes/basic/basic.css">
	<style>
		body {
			padding: 1em;
		}
	</style>
	<title>dojo-widget Scartchpad</title>
</head>
<body class="basic">
	<button type="button" id="next">Close Last Tab</button>
	<!-- Loading the dojo-loader (minified) -->
	<script src="../../../node_modules/dojo-loader/loader.min.js"></script>
	<script>
		require.config({
			baseUrl: '../../..',
			packages: [
				{ name: 'src', location: '_build/src' },
				{ name: 'dojo-actions', location: 'node_modules/dojo-actions' },
				{ name: 'dojo-compose', location: 'node_modules/dojo-compose' },
				{ name: 'dojo-core', location: 'node_modules/dojo-core' },
				{ name: 'immutable', location: 'node_modules/immutable/dist' },
				{ name: 'maquette', location: 'node_modules/maquette/dist' },
				{ name: 'rxjs', location: 'node_modules/@reactivex/rxjs/dist/amd' }
			],
			map: {
				'*': {
					'maquette/maquette': 'maquette/maquette.min',
					'immutable/immutable': 'immutable/immutable.min'
				}
			}
		});

		/* Requiring in the main module */
		require([ 'src/examples/scratch' ], function () {});
	</script>
</body>
</html>
